...
function teamNumbers($name){
    //given the team name return the single number and the compound number
    ...
    $ret = array("name" => $name, "single" => 0, "compound" => 0);
    ...
    for($i=0; $i<strlen($name); $i++){
        $alpha = $name[$i];
        $ret['compound'] = ( $ar[$alpha] + $ret['compound'] );
    }
    //now do single
    $comp = (string)$ret['compound'];
    for($i=0; $i<strlen($comp); $i++){
        $ret['single'] = ( intval($comp[$i]) + $ret['single'] );
    }
    if ( strlen($ret['single']) > 1 ){
        $tmp = (string)$ret['single'];
        $ret['single'] = 0;
        for($i=0; $i<strlen($tmp); $i++){
            $ret['single'] = ( intval($tmp[$i]) + $ret['single'] );
        }
    }
...
function compete($teamone, $teamtwo, $date){
    global $winners, $losers, $ehs, $positives, $negatives, $neutrals, $debug;
    //get the numbers for both teams and determine which is most likely to win on the date they play
    //date should be ISO 8601 (2013-03-19 12:00:00) datetime of the time the game starts in your time zone

    $t1 = teamNumbers($teamone);
    $t2 = teamNumbers($teamtwo);
    ...
    $age = round($moon->age()); //either single or compound
    $distance = round($moon->distance());
    while ( strlen($distance)>1){ //always single
        $tmp = (string)$distance;
        $distance = 0;
        for($i=0; $i<strlen($tmp); $i++){
            $distance = ( intval($tmp[$i]) + $distance );
        }
    }
    $phase = ( 100 * round($moon->phase(), 2)); //always compound
    if ( $phase > 52 ){
        $phase = 52;
    }

    if ( $debug ){
        echo "age: " . $age . ", distance: " . $distance . ", phase: " . $phase . "\n";
    }

    //single is 60%, compound is 40%; date numbers cancel out

    //determine which are cancelled out
    if ( $t1['single'] == $distance ){
        if ( $debug ){
            echo "cancelled out team one single by distance\n";
        }
        $t1['single'] = 0;
    }
    if ( $t2['single'] == $distance ){
        if ( $debug ){
            echo "cancelled out team two single by distance\n";
        }
        $t2['single'] = 0;
    }
    if ( $t1['compound'] == $phase ){
        if ( $debug ){
            echo "cancelled out team one compound by phase\n";
        }
        $t1['compound'] = 0;
    }
    if ( $t2['compound'] == $phase ){
        if ( $debug ){
            echo "cancelled out team two compound by phase\n";
        }
        $t2['compound'] = 0;
    }
    if ( $age <= 9 ){
        if ( $t1['single'] == $age ){
            if ( $debug ){
                echo "cancelled out team one single by age\n";
            }
            $t1['single'] = 0;
        }
        if ( $t2['single'] == $age ){
            if ( $debug ){
                echo "cancelled out team two single by age\n";
            }
            $t2['single'] = 0;
        }
    } else {
        if ( $t1['compound'] == $age ){
            if ( $debug ){
                echo "cancelled out team one compound by age\n";
            }
            $t1['compound'] = 0;
        }
        if ( $t2['compound'] == $age ){
            if ( $debug ){
                echo "cancelled out team two compound by age\n";
            }
            $t2['compound'] = 0;
        }
    }

    //now we compare the team numbers
    $t1flag = 0;
    if ( in_array($t1['single'], $winners) ){
        $t1flag = ( $t1flag + 6 );
    }
    if ( in_array($t1['single'], $losers) ){
        $t1flag = ( $t1flag - 6 );
    }
    if ( in_array($t1['single'], $ehs) ){
        $rand = mt_rand(-5, 5);
        $t1flag = ( $t1flag + $rand );
    }
    if ( in_array($t1['compound'], $positives) ){
        $t1flag = ( $t1flag + 4 );
    }
    if ( in_array($t1['compound'], $negatives) ){
        $t1flag = ( $t1flag - 4 );
    }
    if ( in_array($t1['compound'], $neutrals) ){
        $rand = mt_rand(-5, 5);
        $t1flag = ( $t1flag + $rand );
    }

    $t2flag = 0;
    if ( in_array($t2['single'], $winners) ){
        $t2flag = ( $t2flag + 6 );
    }
    if ( in_array($t2['single'], $losers) ){
        $t2flag = ( $t2flag - 6 );
    }
    if ( in_array($t2['single'], $ehs) ){
        $rand = mt_rand(-5, 5);
        $t2flag = ( $t2flag + $rand );
    }
    if ( in_array($t2['compound'], $positives) ){
        $t2flag = ( $t2flag + 4 );
    }
    if ( in_array($t2['compound'], $negatives) ){
        $t2flag = ( $t2flag - 4 );
    }
    if ( in_array($t2['compound'], $neutrals) ){
        $rand = mt_rand(-5, 5);
        $t2flag = ( $t2flag + $rand );
    }
    ...
    if ( $t1flag > $t2flag ){
        $ret = $t1['name'] . "\n";
    } else if ( $t2flag > $t1flag ){
        $ret = $t2['name'] . "\n";
    } else {
        //tie...let's do some randomness
        $rand = mt_rand(1,2);
        if ( $debug ){
            echo "TIE!\n";
        }
    ...
    if ( $debug ){
        print_r($t1);
        print_r($t2);
        echo "Flag one: " . $t1flag . ", flag two: " . $t2flag . "\n";
    }
